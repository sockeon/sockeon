...
class ScaleFactory
{
    public static function createNamespaceManager(ScaleConfig $config): NamespaceManagerInterface
    {
        if ($config->isRedisRegistry()) {
            return new RedisNamespaceManager($config, RedisFactory::connect($config));
        }
